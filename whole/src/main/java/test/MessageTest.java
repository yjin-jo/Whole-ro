...
public class MessageTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.print("\n력할 메시지함의 소유자 사용자 id를 입력하세요. ");
        printMessageBox(sc.nextLong());

        System.out.print("\n삭제할 메시지 id를 입력하세요 . ");
        if (messageDao.deleteMessage(sc.nextLong())) {
            System.out.println("메시지 삭제에 성공했습니다 ! ");
        }else {
            System.out.println("메시지 삭제에 성공했습니다 ! ");
        }
        
        System.out.print("\n출력할 메시지함의 소유자 사용자 id를 입력하세요. ");
        printMessageBox(sc.nextLong());
    ...
    public static void printMessageBox(Long id) throws SQLException {
        ...
        while (iter.hasNext()) {
            ...
            System.out.println(messageSender + "\t" + messageEntity.getTitle() + "\t"
                    + messageEntity.getRegDate().format(formatter));
        }
    }

    public static void insertMessageBox() throws SQLException {
...
//                LocalDate reg_date, BooleanEnum is_read, MessageType msgType, Long postId);
        MessageEntity insertMessage = new MessageEntity(null, 6L, 2L, "안녕하세요 구매가능할까요?", "글보고연락드려요", "image.jpg",
                LocalDate.now(), myBoolean.FALES, messageType.MARKET, 22L);
